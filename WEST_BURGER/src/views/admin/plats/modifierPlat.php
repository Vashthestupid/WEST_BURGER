...
$selectInfo = "SELECT * FROM plat WHERE id = :id";

$reqInfo = $db->prepare($selectInfo);
$reqInfo->bindParam(':id', $id);
$reqInfo->execute();

$plat = $reqInfo->fetchObject();
...
if(isset($_POST['valider'])){
    ...
    $reqModifier->bindParam(':idPlat', $id);
    $reqModifier->execute();
...
<div class="container">
    <div id="editTypeUser" class="offset-md-2 col-md-8">
        <h2 class=" titleForm d-flex justify-content-center">Modifier un plat</h2>
        <form method="post" class="mt-5">
            <div class="form-group">
                <label for="nom" class="d-flex justify-content-center">Nom du Plat</label>
                <input class="form-inline d-flex mx-auto w-75" type="text" name="nom" id="nom" value="<?= $plat->nomPlat?>">
            </div>
            <div class="form-group">
                <label for="desc" class="d-flex justify-content-center">Description du plat</label>
                <textarea class="form-inline d-flex mx-auto w-75"  name="desc" id="desc" cols="30" rows="10" ><?= $plat->descPlat ?></textarea>
            </div>
            <div class="form-group">
                <label for="prix" class="d-flex justify-content-center">Prix du plat</label>
                <input class="form-inline d-flex mx-auto w-75"  type="number" name="prix" id="prix" value="<?= $plat->prixPlat ?>">
            </div>
            <div class="form-group">
